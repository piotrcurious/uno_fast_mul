    ...
    for (uint8_t col = 0; col < gw; ++col) {
        ...
        for (uint8_t row = 0; row < gh; ++row) {
            if (colbyte & (1 << row)) {
                ...
                // Change the signs for Y-down rotation consistency
                //int32_t rx_q8 = ( (sxs * (int32_t)cos_q15) + (sys * (int32_t)sin_q15) ) >> 15;
                //int32_t ry_q8 = (-(sxs * (int32_t)sin_q15) + (sys * (int32_t)cos_q15) ) >> 15;
...
const uint32_t FLYBY_DURATION = 8800;   // 1.8s per verse
const uint32_t ZOOM_DURATION = 10000;    // 5s zoom out
...
for (int i = 0; i < len; i++) {
        ...
        // 1. Position the glyph in World Space relative to Camera
        float dx = p.x - camX;
        float dy = p.y - camY;
        
        // 2. Rotate the POSITION around the camera
        // We use -camAngle because we are moving the WORLD, not the camera
        ...
        // 3. IMPORTANT: The glyph's rotation is its own orientation 
        // minus the camera's orientation.
        float finalGlyphAngle = p.angle - camAngle;

        draw_glyph_into_tiles(tiles, str[i], screenX, screenY, 
                             p.scale * camZoom, finalGlyphAngle, color);
    }
}
...
// Force the difference into the -PI to +PI range
while (angleDiff > PI)  angleDiff -= TWO_PI;
while (angleDiff < -PI) angleDiff += TWO_PI;

cameraAngle = p1.angle + angleDiff * t;

//cameraAngle = (p1.angle + angleDiff * t)*0.1 + cameraAngle * 0.9; // filter
*/

// 1. Calculate the shortest distance between points (handling the +/- PI wrap)
float targetAngleDiff = p2.angle - p1.angle;
while (targetAngleDiff > PI)  targetAngleDiff -= TWO_PI;
while (targetAngleDiff < -PI) targetAngleDiff += TWO_PI;
float targetAngle = p1.angle + targetAngleDiff * t;

// 2. Calculate the difference between current camera and target
float cameraError = targetAngle - cameraAngle;
while (cameraError > PI)  cameraError -= TWO_PI;
while (cameraError < -PI) cameraError += TWO_PI;

// 3. Apply the filter to the ERROR (the motion), then add to current
// This ensures the camera always takes the shortest path
cameraAngle += cameraError * 0.05f; 

// 4. Final normalization to keep cameraAngle in a sane range
while (cameraAngle > PI)  cameraAngle -= TWO_PI;
while (cameraAngle < -PI) cameraAngle += TWO_PI;
...
void setup() {
    ...
    phaseStartMs = millis();
    // currentPhase = ZOOM_OUT; // test zoom
