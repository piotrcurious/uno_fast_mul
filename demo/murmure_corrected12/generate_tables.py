...
def cpp_escape(s):
    """Escapes a string for use in a C++ double-quoted string literal."""
    result = ""
    for char in s:
        if char == '"':
            result += '\\"'
...
def main():
    ...
    if args.emit_c:
        ...
        if glyph_meta:
            ...
            escaped_chars = cpp_escape("".join(glyph_meta["chars"]))
            c_content.append(f'const char {args.progmem_macro} GLYPH_CHAR_LIST[{len(glyph_meta["chars"])+1}] = "{escaped_chars}";')
            flat_glyphs = []
        ...
        if glyph_meta:
            ...
            escaped_chars = cpp_escape("".join(glyph_meta["chars"]))
            h_content.append(f'const char {args.progmem_macro} GLYPH_CHAR_LIST[{len(glyph_meta["chars"])+1}] = "{escaped_chars}";')
            flat_glyphs = []
            for ch in glyph_meta['chars']:
                cols = glyph_meta['glyphs'][ch]
                flat_glyphs.extend(cols + [0] * (glyph_meta['width'] - len(cols)))
