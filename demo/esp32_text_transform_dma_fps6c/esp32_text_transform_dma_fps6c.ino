...
struct Tile {
  ...
  void init(uint16_t _x0, uint16_t _y0, uint16_t _w, uint16_t _h) {
    ...
    // Force 32-bit alignment and DMA capability
  ...
  void clearTo(uint16_t color = 0x0000) {
    if (!buf) return;
    size_t n = (size_t)w * (size_t)h;
    // Faster than loop for zeroing out
...
struct TileManager {
  ...
  void frameClear(uint16_t bgcolor = 0x0000) {
    ...
    for (uint32_t i=0; i<count; ++i) tiles[i].clearTo(bgcolor);
  }
...
void render_frame() {
    ...
    draw_string_dynamic("COMPOSITOR V3", 5, 10, tft.width(), 50, 2.8f, 20, 20, true, TFT_CYAN);
    ...
    draw_string_dynamic(fps_buf, 0, 190, tft.width(), 240, 8.0f, 20, 10, true, TFT_GREEN);
