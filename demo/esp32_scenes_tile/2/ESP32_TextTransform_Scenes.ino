...
void setup() {
    ...
    for(int i=0; i<count; i++) {
        ...
        tileAnims[i].offsetX = 0;
        tileAnims[i].offsetY = 0;
    }
...
void loop() {
    ...
    if (currentScene == SCENE_EXPLODE) {
        ...
        // Update tile animation states for explosion effect
        for(int ty=0; ty < gTiles.rows; ty++) {
            for(int tx=0; tx < gTiles.cols; tx++) {
                int tileIdx = ty * gTiles.cols + tx;
                
                // Calculate explosion direction from center
                float centerX = gTiles.cols / 2.0f;
                float centerY = gTiles.rows / 2.0f;
                float dx = tx - centerX;
                float dy = ty - centerY;
                
                // Normalize and apply explosion factor
                float dist = sqrt(dx * dx + dy * dy);
                if (dist > 0.1f) {
                    tileAnims[tileIdx].offsetX = (dx / dist) * explodeFactor * TILE_SIZE;
                    tileAnims[tileIdx].offsetY = (dy / dist) * explodeFactor * TILE_SIZE;
                } else {
                    tileAnims[tileIdx].offsetX = 0;
                    tileAnims[tileIdx].offsetY = 0;
                }
            }
        }
    } else {
        // Reset tile offsets when not exploding
        size_t count = gTiles.cols * gTiles.rows;
        for(int i=0; i<count; i++) {
            tileAnims[i].offsetX = 0;
            tileAnims[i].offsetY = 0;
    ...
    for (int i = 0; i < len; i++) {
        ...
        switch(currentScene) {
            ...
            case SCENE_RAIN:
                ...
                break;
            case SCENE_EXPLODE:
                // During explosion, glyphs stay in their positions
                // The visual effect comes from the tile offsets calculated above
                // which would be applied in a more advanced compositor
                // For this implementation, the effect is more conceptual
                angle = explodeFactor * 0.5f;
                scale = 4.5f - (explodeFactor * 0.3f);
                break;
        }
