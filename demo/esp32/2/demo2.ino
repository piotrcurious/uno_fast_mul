...
void advance_scene() {
  // go to next regular scene cyclically (excluding the explode transition)
  current_scene = (Scene)(((int)current_scene + 1) % SCENE_COUNT);
  if (current_scene == SCENE_EXPLODE_TRANSITION) current_scene = (Scene)(((int)current_scene + 1) % SCENE_COUNT);
  scene_start_ms = millis();
}
...
void render_tiles_frame(uint32_t t_ms) {
    ...
    for (int tx = 0; tx < tiles_x; ++tx) {
      ...
      for (int r = 0; r < DEMO_ROWS; ++r) {
        ...
        for (int ci = 0; ci < len; ++ci) {
          ...
          // compute transformed bbox and cull quickly
          float minx, miny, maxx, maxy;
          glyph_transformed_bbox_float(glyph_cx, glyph_cy, sc, ang, minx, miny, maxx, maxy);

          // expand bbox a little
          minx -= 1.0f; miny -= 1.0f; maxx += 1.0f; maxy += 1.0f;

          // apply tile offset for explosion stage
          float bbox_tx0 = (float)tx0 + tile_offset_x;
          float bbox_ty0 = (float)ty0 + tile_offset_y;

          // test intersection
          if (!bbox_intersect((int16_t)floor(minx), (int16_t)floor(miny), (int16_t)ceil(maxx), (int16_t)ceil(maxy),
                              (int16_t)floor(bbox_tx0), (int16_t)floor(bbox_ty0),
                              (int16_t)ceil(bbox_tx0 + TILE_W), (int16_t)ceil(bbox_ty0 + TILE_H))) {
          ...
          rasterize_glyph_to_tile(gidx,
                                  (int16_t)round(glyph_cx + tile_offset_x),
                                  (int16_t)round(glyph_cy + tile_offset_y),
                                  sc, ang, tx0, ty0, 0xFFFF);
          local_dirty = local_dirty || tile_dirty[tile_index];
        } // chars
      } // rows
  ...
  if (!in_transition && (millis() - scene_start_ms) > SCENE_DURATION_MS) {
    ...
    if ((millis() - scene_start_ms) > TRANSITION_MS) {
      // complete transition: advance to next logical scene (the generator next_scene chose explode transition,
      // but we want to go to the next actual scene afterwards)
      // pick next scene sequentially
      current_scene = (Scene)(( (int)current_scene + 1 ) % SCENE_COUNT);
      scene_start_ms = millis();
